...
struct node* send_return(int sockfd, struct sockaddr_in cltaddr, char file_idx, struct node* current, char seq) {
	...
	// Case A: The pointer current is a NULL pointer (i.e. the node list is empty), just return NULL
	if (current == NULL)
	{
		return NULL ;
	}

	// Case B: The node list is not empty, search for the linked node list until find the correct one
	while (current != NULL && (current->file_map & bitmap) == 0)
	{
		current = current->next ;
	}

	// Case C: After loop through the whole node list, none of them match, 
	// or the file we would like to find does not exist,
	// just return NULL
	if (current == NULL)
	{
		return NULL ;
...
int check_timeout(long long now, struct rdt3_sender_ctx *ctx_head, int sockfd) {
    ...
    while (current != NULL) {
		...
		// This part needs double check, easy to implement wrong
		if (current->waiting_ack == 1 && (now - current->clock >= TIMEOUT))
		{
			struct sockaddr_in clientaddr ;
			memset(&clientaddr, 0, sizeof(clientaddr));

			// update the clock time of last packet sending, for timeout use
			...
			send_return(sockfd, clientaddr, 
								  current->file_idx, current->noack_node, current->noack_num) ;
	...
	while (1) {
		...
		if (strncmp(buffer + parse_idx, UPDATE, strlen(UPDATE)) == 0) {
			...
			if (update_node == NULL) {
				...
				// fill send_buf with 0 to clean the things inside
				bzero(send_buf, MAXMSG) ;

				// update the file_map of the corresponding node
				update_node->file_map = new_map ;

				// send an ACK message back (by refer to REGISTER)
				memcpy(send_buf, &seq, sizeof(seq));
				send_idx += 2; /* seq and blank */
