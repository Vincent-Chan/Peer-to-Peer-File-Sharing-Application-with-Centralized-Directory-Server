...
int main(){
    ...
    // Initialize a Packet struct instance
    Packet packet ;
    memset(&packet, 0 ,sizeof(packet)) ;

    // initialize an ack value
    int ack ;
		...
		int poll_count = poll(pfds, fd_count, -1);
			...
			if (client_socket == -1) {
                	...
                	printf("pollserver: new connection from %s on socket %d\n",inet_ntoa(client_addr.sin_addr),client_socket);
                        ...
                        // get the receibed message from the client
                        int nbytes = recvfrom(sockfd, &packet, sizeof(packet), 0, (struct sockaddr*)&client_addr, &addr_size) ;

                        printf("Received packet with seq. no. %d: %s", packet.seq_num, packet.data) ;

                        // update the ack number
                        ack = (packet.seq_num+1) % 2 ;
